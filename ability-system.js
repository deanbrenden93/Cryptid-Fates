...
// ==================== FLAGS ====================
// Flags are boolean properties that can be set on cryptids
// They're used for tracking state and granting abilities
...
class AbilityExecutor {
    constructor(gameState) {
        ...
        this.eventQueue = [];
        this.context = {}; // Current execution context
    }
    
    /**
     * Execute all abilities on a cryptid that match the given trigger
     */
    ...
    checkCondition(condition) {
        ...
        // Get the target to check against
    ...
    /**
     * Execute a single effect on a single target
     */
        ...
        switch (effect.type) {
            ...
            case EffectTypes.CANCEL_ATTACK:
                if (this.context.eventData) {
                    this.context.eventData.cancelled = true;
                }
            ...
            case EffectTypes.REDUCE_DAMAGE:
                // Stores reduction for damage calculation
            ...
            case EffectTypes.GRANT_FLAG:
                // Grant flag to another target (like combatant)
    ...
    /**
     * Resolve a target type to actual cryptid(s)
     */
    resolveTarget(targetType) {
        if (!targetType) return [this.context.self];
        
        // If it's already a cryptid object, return it
        if (typeof targetType === 'object' && targetType.key) {
            return [targetType];
        }
        ...
        switch (targetType) {
            ...
            case TargetTypes.OWNER:
                return [{ isPlayer: owner === 'player', owner }]; // For pyre/draw effects
                
            case TargetTypes.MY_COMBATANT:
                return [this.getCombatant(self)].filter(Boolean);
                
            case TargetTypes.MY_SUPPORT:
                return [this.getSupport(self)].filter(Boolean);
                
            case TargetTypes.ENEMY_ACROSS:
                return this.getEnemiesAcross(self).slice(0, 1);
                
            case TargetTypes.ENEMIES_ACROSS:
                return this.getEnemiesAcross(self);
                
            case TargetTypes.ALL_ALLIES:
                return this.getAllCryptids(owner);
                
            case TargetTypes.ALL_ENEMIES:
                return this.getAllCryptids(owner === 'player' ? 'enemy' : 'player');
                
            case TargetTypes.ALL_MY_COMBATANTS:
                return this.getAllCombatants(owner);
                
            case TargetTypes.ALL_ENEMY_COMBATANTS:
                return this.getAllCombatants(owner === 'player' ? 'enemy' : 'player');
            ...
            case TargetTypes.ATTACKER:
                return [this.context.attacker || this.context.eventData?.attacker].filter(Boolean);
                
            case TargetTypes.DEFENDER:
                return [this.context.defender || this.context.eventData?.defender].filter(Boolean);
                
            case TargetTypes.TARGET:
                return [this.context.target || this.context.eventData?.target].filter(Boolean);
                
            case TargetTypes.VICTIM:
                return [this.context.victim || this.context.eventData?.victim].filter(Boolean);
                
            case TargetTypes.KILLER:
                return [this.context.killer || this.context.eventData?.killer].filter(Boolean);
                
            case TargetTypes.ADJACENT:
                return this.getAdjacentCryptids(self);
            ...
            case TargetTypes.EACH_ENEMY:
                return this.getAllCryptids(owner === 'player' ? 'enemy' : 'player');
                
            case TargetTypes.EACH_ALLY:
                return this.getAllCryptids(owner);
    ...
    /**
     * Resolve a value (can be static number or calculation)
     */
    resolveValue(value, target) {
        if (value === undefined || value === null) return 0;
        if (typeof value === 'number') return value;
        
        // Object with calc property
        if (typeof value === 'object' && value.calc) {
            const calcTarget = value.of 
                ? this.resolveTarget(value.of)[0] 
                : target || this.context.self;
    ...
    calculate(calcType, target, params = {}) {
        switch (calcType) {
            case CalcTypes.AILMENT_STACKS:
                return this.countAilmentStacks(target);
                
            case CalcTypes.BURN_TURNS:
                return target?.burnTurns || 0;
                
            case CalcTypes.BLEED_TURNS:
                return target?.bleedTurns || 0;
                
            case CalcTypes.CALAMITY_COUNTERS:
                return target?.calamityCounters || 0;
                
            case CalcTypes.CURSE_TOKENS:
                return target?.curseTokens || 0;
                
            case CalcTypes.AILMENTED_ENEMY_COUNT:
                return this.countAilmentedEnemies(this.context.owner);
                
            case CalcTypes.VAMPIRE_COUNT:
                return this.countByName(this.context.owner, 'vampire');
                
            case CalcTypes.GARGOYLE_COUNT:
                return this.countByName(this.context.owner, 'gargoyle');
                
            case CalcTypes.ALLY_DEATHS_LAST_TURN:
                return this.state.deathsLastEnemyTurn?.[this.context.owner] || 0;
                
            case CalcTypes.UNIQUE_AILMENTS:
                return this.countUniqueAilments(target);
                
            case CalcTypes.TARGET_COST:
                return target?.cost || 0;
                
            case CalcTypes.TARGET_BASE_HP:
                return target?.hp || 0;
                
            case CalcTypes.HALF_ROUNDED_DOWN:
                const baseValue = this.resolveValue(params.of, target);
                return Math.floor(baseValue / 2);
                
            case CalcTypes.MIN:
                return Math.min(
                    this.resolveValue(params.a, target),
                    params.max || Infinity
                );
            ...
            default:
                console.warn(`Unknown calc type: ${calcType}`);
                return 0;
        }
    }
    ...
    hasAnyAilment(target) {
        ...
        return (target.burnTurns || 0) > 0 ||
               (target.bleedTurns || 0) > 0 ||
               !!target.paralyzed ||
               (target.paralyzeTurns || 0) > 0 ||
               (target.calamityCounters || 0) > 0 ||
               (target.curseTokens || 0) > 0;
    }
    ...
    countAilmentedEnemies(owner) {
        ...
        return enemies.filter(e => this.hasAnyAilment(e)).length;
    }
    ...
    // These should be overridden or injected to match your game state structure
    
    getCombatCol(owner) {
        // Default: player combat is col 1, enemy combat is col 0
    ...
    getEnemiesAcross(cryptid) {
        ...
        // Combatant across
        const combatant = this.getCryptidAt(enemyOwner, enemyCombatCol, cryptid.row);
        if (combatant) result.push(combatant);
        
        // Support across
        const supportCol = this.getSupportCol(enemyOwner);
        const support = this.getCryptidAt(enemyOwner, supportCol, cryptid.row);
    ...
    // ==================== GAME ACTIONS ====================
    // These perform actual game state mutations
    ...
    dealDamage(target, amount, source = 'ability') {
        ...
        this.emitEvent('onDamageTaken', {
            target,
            damage: amount,
            source: this.context.self,
            sourceType: source,
            hpBefore,
    ...
    cleanse(target) {
        ...
        if (hadAilments > 0) {
            this.emitEvent('onCleanse', { cryptid: target, count: hadAilments, owner: target.owner });
        }
    ...
    gainPyre(owner, amount) {
        ...
        if (owner === 'player') {
            this.state.playerPyre = (this.state.playerPyre || 0) + amount;
        } else {
            this.state.enemyPyre = (this.state.enemyPyre || 0) + amount;
        }
    ...
    stealPyre(owner, amount) {
        ...
        if (enemyOwner === 'player') {
            this.state.playerPyre -= stolen;
        } else {
            this.state.enemyPyre -= stolen;
        }
    ...
    killCryptid(target, killedBy = 'ability') {
        ...
        if (field?.[target.col]?.[target.row] === target) {
            field[target.col][target.row] = null;
        }
        
        this.emitEvent('onDeath', {
            cryptid: target,
            owner: target.owner,
            col: target.col,
            row: target.row,
            killedBy,
            killedBySource: this.context.self
        });
